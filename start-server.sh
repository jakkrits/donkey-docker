...
HASIMAGE=$( docker image ls | grep donkeyserver | wc -l )
if [ $HASIMAGE == 0 ]; then
	echo "start-server: Building Donkey server image..." >&2
    docker build --file Dockerfile.server -t donkeyserver .
fi
...
while getopts ":vbd" opt; do
  case $opt in
    ...
    d)
		...
		docker run -p 8887:8887 -v ~/mydonkey:/root/mydonkey --entrypoint=/bin/bash -it donkeyserver
		exit
		;;
    \?)
...
docker run -p 8887:8887 -v ~/mydonkey:/root/mydonkey donkeyserver
